...
public class Main extends JFrame {
	...
	ArrayList<ArrayList<Integer>> B = new ArrayList<ArrayList<Integer>>(); // Ursprünglicher
																			// Zustand
																			// Vorheriger
																			// -1,0,1,2
	// Integer[][] B = new Integer[sizeX][sizeX];
	ArrayList<ArrayList<Integer>> B2 = new ArrayList<ArrayList<Integer>>(); // Zustand
																			// bei
																			// T+1
																			// Jetziger
																			// -1,0,1,2
	ArrayList<ArrayList<Integer>> O = new ArrayList<ArrayList<Integer>>(); // Seegras
																			// 0,1
	ArrayList<ArrayList<ArrayList<Double>>> b = new ArrayList<ArrayList<ArrayList<Double>>>(); // Wind
																								// in
																								// Koordinaten
																								// in
																								// 6
	...
	private volatile ArrayList<Pixel> pA = new ArrayList<Pixel>(); // Array mit allen
															// Feldern
	...
	public void createGUI() {
		...
		rbDisabled.addMouseListener(new radioMouseListener());//TODO do
	...
	public synchronized void generateField() {
		...
		for (int y = 0; y < sizeX; y++) {
			...
			for (int x = 0; x < sizeX; x++) {
				...
				/*
				 * for(int x=0;x<O.size();x++){ for (int
				 * y=0;y<O.get(x).size();y++){
				...
				/*
				 * for (int i=0;i<4;i++){ int j=(int) Math.round(Math.random());
				 * if (y==0||y==sizeX-1||x==0||x==sizeX-1){
				 * b.get(y).get(x).set(i+4, (double)-1);
				 * b.get(y).get(x).set(i,(double)-1); }else{ if (j==1){
				 * b.get(y).get(x).set(i, Math.random()/6);
				 * b.get(y).get(x).set(i+4,(double) 0); } if (j==0){
	...
	public synchronized void colorField() {
		...
		for (ArrayList<Integer> i : B) {
			for (Integer k : i) {
				pA.add(z, new Pixel(/*x,y,*/k, sizeP));
	...
	public synchronized void getNextState() {
		...
		for (Pixel P : pA) {
			...
			//System.out.println((Math.round((long) (Math.floor(l / sizeX))))+","+(l % sizeX));
		...
		for (int y = 0; y < sizeX; y++) {
			...
			for (int x = 0; x < sizeX; x++) {
				...
				if (B.get(y).get(x) < 0) {
					...
					/* 
					 * if (Math.round((int)
					 * ((g*Math.sin((t+5)*Math.PI/6)+0.4+o*O.get(y).get(x))*B.
					 * get(y).get(x)))<0){ B2.get(y).set(x,0); debug(
					 * "seefehler "+(Math.round((int)
					 * ((g*Math.sin((t+5)*Math.PI/6)+0.4+o*O.get(y).get(x))*B.
					 * get(y).get(x))))+" mit 0 beglichen"); }else
					 */ 
						B2.get(y)
								.set(x, Math.round(
										(int) (((g * Math.sin((t + 5) * Math.PI / 6) + 0.4) + o * O.get(y).get(x))
												* B.get(y).get(x))));
				}else{
					// debug(B.get(y).get(x));
					...
					//System.out.println(B);
					// B2[y][x]=(int)
					// (((g*Math.sin((5)/6)+0.4)+o*O[y][x])*B[y][x]*2.5);
					
					B2.get(y).set(x, Math.round(
							(int) (((g * Math.sin((t + 5) * Math.PI / 6) + 0.4) + o * O.get(y).get(x)) * B.get(y).get(x)
					...
					/*
					 * if (B2.get(y).get(x)==-1){ a++; } if
					 * (B2.get(y).get(x)==0){ k++;} if (B2.get(y).get(x)==1){
					 * c++;} if (B2.get(y).get(x)==2){ d++;} if
					 * (B2.get(y).get(x)==0){
					 * 
					 * debug("B:"+B.get(y).get(x));
					 * debug("B2:"+(((g*Math.sin((t+5)*Math.PI/6)+0.4)+o*O.get(y
					 * ).get(x))*B.get(y).get(x)
					 * 
					 * + b.get(y).get(x).get(6)*B.get(y).get(x+1) //rechts +
					 * b.get(y).get(x).get(2)*B.get(y).get(x-1) //links +
					 * b.get(y).get(x).get(0)*B.get(y+1).get(x) //unten +
					 * b.get(y).get(x).get(4)*B.get(y-1).get(x)
					 * //ob.get(y).get(x).get(en
					 * 
					 * + (1/Math.sqrt(2)) *
					 * b.get(y).get(x).get(7)*B.get(y+1).get(x+1) +
					 * (1/Math.sqrt(2)) *
					 * b.get(y).get(x).get(5)*B.get(y-1).get(x+1) +
					 * (1/Math.sqrt(2)) *
					 * b.get(y).get(x).get(1)*B.get(y+1).get(x-1) +
					 * (1/Math.sqrt(2)) *
					 * b.get(y).get(x).get(3)*B.get(y-1).get(x-1)) );
					 * debug("x:"+x); debug("y:"+y);
					 * debug("alpha:"+g*Math.sin((t+5)*Math.PI/6));
					 * debug("seegras:"+o*O.get(y).get(x)); debug("beta:"+(
					 * b.get(y).get(x).get(6)*B.get(y).get(x+1) //rechts +
					 * b.get(y).get(x).get(2)*B.get(y).get(x-1) //links +
					 * b.get(y).get(x).get(0)*B.get(y+1).get(x) //unten +
					 * b.get(y).get(x).get(4)*B.get(y-1).get(x)
		...
		public void run() {
			while (true) {
				...
				if (sizeP * sizeX >= 120 && sizeP * sizeX < screenSize.getHeight() - 20 && sizeP > 2) {
					...
					try {
						/*
						 * for(Pixel p:pA){ p.setSize(sizeP);
						 * p.setPreferredSize(new Dimension(sizeP,sizeP));
						 * p.repaint(); }
						 */
						...
						if (sizeX * sizeX < pA.size()) {
							// pixelFrame.setSize(new
							// Dimension(sizeX*sizeP,sizeX*sizeP));
							/*
							 * for (Pixel p:pA){ jP.remove(p); }
							 */
							jP.removeAll();
							generateField();
							colorField();
	...
	private class TextFieldFocusListener implements FocusListener {
		...
		public void focusGained(FocusEvent arg0) {
			...
			if (((JTextComponent) arg0.getSource()).getText().length() > 10) {
		...
		public void focusLost(FocusEvent arg0) {
			if (((JTextComponent) arg0.getSource()).getText().length() < 1) {
				((JTextComponent) arg0.getSource()).setText(((JTextComponent) arg0.getSource()).getToolTipText());
			} else {
				textfieldTemp = null;
			}
		}
...
*/
	...
	JRadioButton lastOne;

	@Override
	public void mouseClicked(MouseEvent arg0) {
		System.out.println("clicked");
		if ((JRadioButton)(arg0.getSource())==lastOne){
			
			if (((Pixel) arg0.getSource()).getMeColor() != -2) {
				...
				if (hashCode == rbBrown.getModel().hashCode()) {
					((Pixel) arg0.getSource()).setColor(-2);
				} else if (hashCode == rbBlue.getModel().hashCode()) {
					((Pixel) arg0.getSource()).setColor(0);
				} else if (hashCode == rbLGreen.getModel().hashCode()) {
					((Pixel) arg0.getSource()).setColor(1);
				} else if (hashCode == rbDGreen.getModel().hashCode()) { // Radiobutton
																			// Grün
																			// ->
																			// setze
																			// Farbe
			...
			lastOne=(JRadioButton) arg0.getSource();
		}
	...
	public class pixelClickMouseListener implements MouseListener {

		@Override
		public void mouseClicked(MouseEvent arg0) {

			((Pixel) arg0.getComponent()).setColor(3);
			((Pixel) arg0.getComponent()).repaint();
		}
